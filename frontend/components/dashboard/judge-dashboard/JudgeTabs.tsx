...
const JudgeTabs = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "pending":
        return <div>القضايا المعلقة</div>
      case "hearings":
        return <div>الجلسات</div>
      case "decisions":
        return <div>القرارات</div>
      case "translation":
  ...
  return (
    <div className="flex gap-6 relative">
      <div className="hidden lg:block lg:w-64">
        <Sidebar role="judge" activeTab={activeTab} onTabChange={handleTabChange} />
      </div>
        ...
        <div className="lg:hidden fixed inset-0 z-50 bg-black bg-opacity-50 flex">
          <aside className="w-80 max-w-[85vw] bg-white dark:bg-gray-800 overflow-y-auto relative">
            <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700">
              <h2 className="text-lg font-bold">لوحة القاضي</h2>
              <button
      ...
      <div className="flex-1 bg-white dark:bg-gray-800 rounded-lg shadow p-4">
        <h1 className="text-2xl font-bold mb-4">لوحة القاضي</h1>
        {renderTabContent()}
      </div>
    </div>
  )
